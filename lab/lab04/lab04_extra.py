...
def create_board(rows, columns):
    ...
    """
    "*** YOUR CODE HERE ***"
    #return rows*[create_row(columns)] # 这里不能这样用的原因是，指针问题
...
def put_piece(board, max_rows, column, player):
    ...
    #new_col = [board[i][column] for i in range(max_rows)]
...
def make_move(board, max_rows, max_cols, col, player):
    ...
    """
    "*** YOUR CODE HERE ***"
    if col not in list(range(max_cols)):
        return -1, board
    else:
        for i in range(max_rows-1,-1,-1):
            if board[i][col] == '-':
...
def print_board(board, max_rows, max_cols):
    ...
    for row in range(max_rows):
        print(''.join([board[row][col]+' ' for col in range(max_cols)]).strip())
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    "*** YOUR CODE HERE ***"
    count = 0
    for elem in [board[i][col] for i in range(max_rows)]:
        if elem == player:
            count+=1
            if count == num_connect:
                return True
